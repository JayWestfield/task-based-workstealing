...
#include <utility>
#include <vector>
template <typename TaskContext> class ChaseLevDeque_v3_fallback {

public:
  ChaseLevDeque_v3_fallback(size_t capacity = 1024)
      : capacity_(capacity), top_(0), bottom_(0), mask(capacity - 1) {
    tasks_ = new Task_v3<TaskContext> *[capacity];
  }

  ~ChaseLevDeque_v3_fallback() { delete[] tasks_; }
  ...
  void push(Task_v3<TaskContext> *task) {
    ...
    if (size >= static_cast<int>(capacity_) - 1 && size > 0) {
      overflowTasks_.push_back(reinterpret_cast<Task_v3<TaskContext> *>(task));
    } else {
...
#pragma omp simd
  ...
  Task_v3<TaskContext> *pop() {
    ...
    if (size < 0) {
      ...
      if (!overflowTasks_.empty()) {
        auto value = overflowTasks_.back();
        overflowTasks_.pop_back();
        return value;
      }
...
private:
  size_t capacity_;
  std::vector<Task_v3<TaskContext> *> overflowTasks_;
  std::atomic<size_t> top_, bottom_;
  Task_v3<TaskContext> **tasks_;
  size_t mask;
};

template <typename TaskContext> class ChaseLevOwnQueue_v3_fallback {
public:
  ChaseLevOwnQueue_v3_fallback(uint64_t numThreads)
...
private:
  ...
  std::vector<ChaseLevDeque_v3_fallback<TaskContext>> taskQueues_;
};