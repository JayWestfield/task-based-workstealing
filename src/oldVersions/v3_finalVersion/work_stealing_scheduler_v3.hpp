...
class WorkStealingScheduler_v3 {

public:
  WorkStealingScheduler_v3(
      uint64_t numThreads, std::function<void()> idleFunction,
  ...
  void workerThread(uint16_t thread_index) {
    ...
    while (!joinThreads_) {
      ...
      while (!stopFlag_) {
        ...
        Task_v3<TaskContext> *nextTask = taskManager_.getTask();
        if (nextTask != nullptr) {
